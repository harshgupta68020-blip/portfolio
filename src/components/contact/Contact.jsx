...
import { MagneticElement } from '@/components/motion/MicroInteractions';
...
export default function Contact() {
  ...
  return (
    <footer id="contact" className="py-32 md:py-40 px-4 max-w-[1400px] mx-auto text-center border-t border-white/10">
      ...
      >
        {/* M2: Scroll-Scrubbed Headline */}
        <ScrubbedWordReveal
          as="h2"
          text="Let's build something meaningful."
          start="top 90%"
          end="center 50%"
          className="text-white font-semibold text-[clamp(2.25rem,5vw,5rem)] tracking-[-0.03em] max-w-[900px] leading-tight"
        />

        {/* Minimal Pill Links with M6 Magnetic Pull */}
        <div className="flex flex-wrap items-center justify-center gap-4 mt-4">
          {contactLinks.map((item, idx) => {
            const IconComp = item.icon;
            return (
              <MagneticElement key={idx} radius={60} strength={0.25}>
                <a
                  href={item.href}
        ...
        <div className="mt-16 text-xs font-mono text-white/40">
          © {new Date().getFullYear()} Harsh. All rights reserved. Cinematic Minimalism v2.1.
        </div>
      </motion.div>
    </footer>
  );
