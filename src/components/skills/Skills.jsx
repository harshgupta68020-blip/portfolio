...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-32 md:py-40 px-4 max-w-[1400px] mx-auto">
      ...
      {/* 4-Column Card Grid with M6 Spotlight Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {skillsData.map((category) => {
          ...
          return (
            <SpotlightCard
              key={category.id}
              className="bg-white/[0.03] border border-white/10 hover:border-[#3B82F6]/40 rounded-[24px] transition-all duration-300 backdrop-blur-sm min-h-[360px] cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-[#3B82F6]"
            >
              <motion.div
                {...cardHoverProps}
